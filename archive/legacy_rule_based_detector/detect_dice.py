...
# Path to the calibrated ROI written by calibrate_tray_roi.py
_TRAY_ROI_PATH = os.path.join(
...
def find_tray_roi(frame: np.ndarray) -> tuple[int,int,int,int] | None:
    """
    Return the dice tray's INNER playing surface as (x, y, w, h).

    Strategy:
      1. **Calibrated path (preferred):** if tray_roi.json exists, load the
         saved bounding box. The rig is mechanically rigid so the tray's
         pixel coordinates don't change between rolls — calibrating once
         (via calibrate_tray_roi.py) is faster and more robust than
         re-detecting every frame, and works in IR mode where the tray
         loses its red color.

      2. **Red-detection fallback:** if no calibration file exists, fall
         back to detecting the red NAF tray walls and shrinking by
         TRAY_WALL_FRACTION. Only works in day mode with the red tray.
    """
    # ── Calibrated path ──────────────────────────────────────────────────────
    if os.path.exists(_TRAY_ROI_PATH):
        try:
            with open(_TRAY_ROI_PATH) as f:
                roi = json.load(f)
            fh, fw = frame.shape[:2]
            # Sanity check — calibration must match current resolution
            if (roi.get("frame_width") == fw and
                    roi.get("frame_height") == fh):
                return (int(roi["x"]), int(roi["y"]),
                        int(roi["w"]), int(roi["h"]))
            # Resolution mismatch — fall through to red detection
        except (OSError, ValueError, KeyError):
            pass   # fall through

    # ── Red-detection fallback (legacy path) ─────────────────────────────────
